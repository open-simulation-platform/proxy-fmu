...
{
    auto damper = loadFmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Damper.fmu");
    auto mass = loadFmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Mass.fmu");
    auto spring = loadFmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Spring.fmu");
    test(damper.get(), mass.get(), spring.get());
}

TEST_CASE("client_test_mass-spring-damper")
{
    auto damper = client::proxy_fmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Damper.fmu");
    auto mass = client::proxy_fmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Mass.fmu");
    auto spring = client::proxy_fmu(std::string(PROXYFMU_DATA_DIR) + "/fmus/1.0/Spring.fmu");
